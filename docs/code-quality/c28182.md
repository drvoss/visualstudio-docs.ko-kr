...
ms.locfileid: "72015106"
---
# <a name="c28182"></a>C28182
경고 C28182: NULL 포인터를 역참조하고 있습니다.

 **추가 정보**: *\<pointer1 >* *\<pointer2 >* 에 대 한 동일한 NULL 값을 포함 @no__t *>*
 ...
 *\<pointer2 >* 는 잠재적으로 NULL로 확인 됩니다. *\<pointer1 >* 는 *pointer2* 와 같은 값을 포함 하 고 역참조 됩니다. 이러한 포인터는 코드에서 매우 다른 위치에 있을 수 있으므로, 코드 분석 도구에서 이 경고를 보고하는 이유를 확인할 수 있도록 두 포인터가 모두 보고됩니다.

 확인 되지 않은 이전 조건 인스턴스가 존재 하는 경우 *\<note >* 다음 텍스트로 바뀝니다. "이 문제가 발생할 수 있는 이전 위치는 줄 *\<number >* 를 참조 하세요."
