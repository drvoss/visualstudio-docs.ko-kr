...
## <a name="example"></a>예제
 다음 예제에서 코드 분석 도구는 `if` 문에서 *s* 가 NULL이 고 `if`의 본문이 사용 되는지 확인 합니다. 그러면 포인터가 *t* 에 *할당 된 다음* *t* 가 `NULL` 역참조가 발생할 수 있는 방식으로 사용 됩니다.
 ...
 **d: \ sample\testfile.cpp (38): warning C28183: ' 0 ' 일 수 있으며 ' s '에 있는 값의 복사본입니다 .이는 ' lstrlenA ' 함수의 사양을 따르지 않습니다.: 가로줄 31, 32, 36, 38**