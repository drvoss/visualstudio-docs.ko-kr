...
ms.locfileid: "72445061"
---
# <a name="c6220"></a>C6220
경고 C6220-의미 체계가 다른 정수 형식 간의 암시적 캐스트입니다. HRESULT를-1과 비교 합니다. SUCCEEDED 또는 FAILED 매크로를 대신 사용 하십시오.

 이 경고는 `HRESULT` `HRESULT` 올바른 형식의 `HRESULT` 아닌-1의 명시적 및 비-값을 비교 하 고 있음을 나타냅니다. @No__t_0 (`E_FAIL`)의 오류는-1로 표시 되지 않습니다. 따라서 `HRESULT` 정수로의 암시적 캐스트는 잘못 된 값을 생성 하 고 잘못 된 결과로 이어질 가능성이 높습니다.
...
```
 ...
 @No__t_0 또는 `FAILED` 매크로를 사용 하 여 `HRESULT` 값을 테스트 하는 것이 가장 좋습니다. 이 경고를 해결 하려면 다음 코드를 사용 합니다.
