...
ms.locfileid: "72441801"
---
# <a name="c6221"></a>C6221
경고 C6221: 의미 체계가 다른 정수 형식 간의 암시적 캐스트입니다. HRESULT를 정수와 비교 하 고 있습니다. SUCCEEDED 또는 FAILED 매크로를 대신 사용 하는 것이 좋습니다.

 이 경고는 `HRESULT` 0이 아닌 정수와 비교 됨을 나타냅니다. @No__t_0 (`S_OK`)의 성공은 0으로 표시 됩니다. 따라서 `HRESULT` 정수로의 암시적 캐스트는 잘못 된 값을 생성 하 고 잘못 된 결과로 이어질 가능성이 높습니다. 실제로 `HRESULT` 반환 될 때 정수를 반환 하는 함수를 실수로 예상 하 여 발생 하는 경우가 많습니다.
