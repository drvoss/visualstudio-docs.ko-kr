...
ms.locfileid: "72018127"
---
# <a name="c6236"></a>C6236
경고 C6236: (\<expression > &#124; &#124; \<non이 아닌 상수 >)는 항상 0이 아닌 상수입니다.
...
## <a name="example"></a>예제
 이 코드는 경고 C6236를 표시 하는 방법을 보여 줍니다. @No__t-0이 0이 아니기 때문에-1 @no__t은 항상 0이 아니고 `else` 절은 실행 되지 않습니다. 그러나 `INPUT_TYPE`은 1이 아닌 값을 사용 하는 상수 이며이 값은 비교에 대 한 값으로 사용 되는 것으로 제안 합니다.
...
```
 ...
 다음 코드에서는 비트 and (`&`) 연산자를 대신 사용 하 여 입력 매개 변수 `n`의 `INPUT_TYPE` 비트가 설정 되었는지 여부를 테스트 합니다.
