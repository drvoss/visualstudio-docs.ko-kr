...
클래스는 소유 하 고 있는 관리 되지 않는 리소스를 삭제 하기 위해 <xref:System.IDisposable> 인터페이스를 구현 합니다. @No__t-0 유형인 인스턴스 필드는 관리 되지 않는 리소스를 소유 하 고 있음을 나타냅니다. @No__t-0 필드를 선언 하는 클래스는 관리 되지 않는 리소스를 간접적으로 소유 하 고 <xref:System.IDisposable> 인터페이스를 구현 해야 합니다. 클래스에서 관리 되지 않는 리소스를 직접 소유 하지 않는 경우 종료자를 구현 하지 않아야 합니다.

## <a name="how-to-fix-violations"></a>위반 문제를 해결하는 방법
이 규칙 위반 문제를 해결 하려면 <xref:System.IDisposable>을 구현 하 고 <xref:System.IDisposable.Dispose%2A?displayProperty=fullName> 메서드에서 필드의 <xref:System.IDisposable.Dispose%2A> 메서드를 호출 합니다.
