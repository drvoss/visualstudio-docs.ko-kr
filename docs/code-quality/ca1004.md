...
## <a name="example"></a>예
다음 예제에서는 두 개의 제네릭 메서드를 호출 하는 구문을 보여 줍니다. `InferredTypeArgument`에 대 한 형식 인수가 유추 되 고 `NotInferredTypeArgument`에 대 한 형식 인수를 명시적으로 지정 해야 합니다.
