...
<xref:System.FlagsAttribute?displayProperty=fullName> 적용 되지 않은 열거형은 0 값을 가진 멤버를 정의 하지 않습니다. 또는 <xref:System.FlagsAttribute> 적용 된 열거형에서 값이 0 인 멤버를 정의 하지만 해당 이름이 ' n o n e '이 아닙니다. 또는 열거형은 0이 아닌 여러 개의 멤버를 정의 합니다.
...
<xref:System.FlagsAttribute> 적용 된 열거가 반환 하는 멤버를 정의 하는 경우 열거형에 값이 설정 되지 않았음을 나타내려면 해당 이름이 ' n o n e ' 이어야 합니다. 다른 용도에 대해 0 값 멤버를 사용 하는 것은 AND 및 OR 비트 연산자가 멤버와 쓸모가 없다는 점에서 <xref:System.FlagsAttribute>를 사용 하는 것과는 반대입니다. 이는 한 멤버만 값 0을 할당 해야 함을 의미 합니다. 플래그 특성이 지정 된 열거형에서 값이 0 인 멤버가 여러 개 발생 하는 경우 `Enum.ToString()`은 0이 아닌 멤버에 대해 잘못 된 결과를 반환 합니다.
...
다음 예제에서는 규칙을 만족 하는 열거형 두 개와 규칙을 위반 하는 열거형 `BadTraceOptions`를 보여 줍니다.
