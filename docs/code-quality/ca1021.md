...
반환 값은 일반적이 고 많이 사용 되지만 `out` 및 `ref` 매개 변수의 올바른 응용 프로그램에는 중간 디자인 및 코딩 기술이 필요 합니다. 일반 사용자를 위해 디자인 한 라이브러리 설계자는 사용자가 `out` 또는 `ref` 매개 변수로 작업 하는 것을 예측할 수 없습니다.
...
<xref:System.Int32.TryParse%2A?displayProperty=fullName>등의 **Try\<** 패턴을 구현 하는 메서드는이 위반을 발생 시 키 지 않습니다. 다음 예제에서는 <xref:System.Int32.TryParse%2A?displayProperty=fullName> 메서드를 구현 하는 구조체 (값 형식)를 보여 줍니다.
