...
사용자 지정 정렬 순서를 정의 하는 형식은 <xref:System.IComparable> 인터페이스를 구현 합니다. <xref:System.IComparable.CompareTo%2A> 메서드는 형식의 두 인스턴스에 대 한 올바른 정렬 순서를 나타내는 정수 값을 반환 합니다. 이 규칙은 정렬 순서를 설정 하는 형식을 식별 합니다. 정렬 순서를 설정 하는 것은 같음, 같지 않음, 보다 작음 및 보다 큼의 일반적인 의미가 적용 되지 않는다는 것을 의미 합니다. <xref:System.IComparable>의 구현을 제공 하는 경우 일반적으로 <xref:System.IComparable.CompareTo%2A>와 일치 하는 값을 반환 하도록 <xref:System.Object.Equals%2A>를 재정의 해야 합니다. <xref:System.Object.Equals%2A>를 재정의 하 고 연산자 오버 로드를 지 원하는 언어로 코딩 하는 경우 <xref:System.Object.Equals%2A>와 일치 하는 연산자도 제공 해야 합니다.
