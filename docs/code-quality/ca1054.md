...
형식이 "uri", "Uri", "urn", "Urn", "url" 또는 "Url"을 포함 하는 문자열 매개 변수를 포함 하는 메서드를 선언 합니다 .이 형식은 <xref:System.Uri?displayProperty=fullName> 매개 변수를 사용 하는 해당 오버 로드를 선언 하지 않습니다.
...
이 규칙 위반 문제를 해결 하려면 매개 변수를 <xref:System.Uri> 형식으로 변경 합니다. 이는 주요 변경 내용입니다. 또는 <xref:System.Uri> 매개 변수를 사용 하는 메서드의 오버 로드를 제공 합니다. 이는 주요 변경 내용입니다.
