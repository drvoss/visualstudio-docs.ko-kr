...
예외는 <xref:System> 네임 스페이스의 서브 클래스 중 하나 또는 <xref:System.Exception?displayProperty=fullName>에서 파생 되어야 합니다.
...
<xref:System.ApplicationException>에 대 한 위반에 대해서는이 규칙의 경고를 표시 하지 마십시오. <xref:System.Xml.XmlDocument>에 대 한 위반에 대 한 경고를이 규칙에서 무시 해도 안전 합니다. 코드가 이전에 릴리스된 경우 제네릭이 아닌 컬렉션에 대 한 경고를 표시 하지 않는 것이 안전 합니다.
