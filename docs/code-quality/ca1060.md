...
<xref:System.Runtime.InteropServices.DllImportAttribute?displayProperty=fullName> 특성을 사용 하 여 표시 된 것과 같은 플랫폼 호출 메서드 또는 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]의 `Declare` 키워드를 사용 하 여 정의 된 메서드를 사용 하 여 비관리 코드에 액세스할 수 있습니다. 이러한 메서드는 다음 클래스 중 하나에 있어야 합니다.
