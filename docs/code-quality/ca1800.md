...
C# `is` 연산자를 사용 하 여 실제 캐스트를 수행 하기 전에 캐스팅이 성공할 지 여부를 테스트 하는 경우에는 `as` 연산자의 결과를 대신 테스트 하는 것이 좋습니다. 이는 `is` 연산자에 의해 수행 되는 암시적 캐스트 작업 없이 동일한 기능을 제공 합니다. 또는 C# 7.0 이상에서는 [패턴 일치](/dotnet/csharp/language-reference/keywords/is#pattern-matching-with-is) 와 함께 `is` 연산자를 사용 하 여 형식 변환을 확인 하 고 식을 한 단계에서 해당 형식의 변수로 캐스트 합니다.
