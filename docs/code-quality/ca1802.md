...
필드는 `static` 및 `readonly` (`Shared` 및 @no__t-Visual Basic 3)로 선언 되 고 컴파일 시간에 계산할 수 값으로 초기화 됩니다.

기본적으로이 규칙은 외부에서 볼 수 있는 필드만 볼 수 있지만이는 [구성 가능](#configurability)합니다.

## <a name="rule-description"></a>규칙 설명

@No__t-0 필드의 값은 선언 형식의 정적 생성자가 호출 될 때 런타임에 계산 됩니다. @No__t-0 필드가 선언 될 때 초기화 되 고 정적 생성자가 명시적으로 선언 되지 않은 경우 컴파일러는 정적 생성자를 내보내 필드를 초기화 합니다.

@No__t-0 필드의 값은 컴파일 시간에 계산 되 고 메타 데이터에 저장 되며,이 값은 `static readonly` 필드와 비교할 때 런타임 성능을 향상 시킵니다.

대상 필드에 할당 된 값은 컴파일 시간에 계산할 수 이므로 값이 런타임에 대신 컴파일 시간에 계산 되도록 선언을 `const` 필드로 변경 합니다.
...
다음 예제에서는 규칙을 위반 하는 `UseReadOnly`과 규칙을 충족 하는 형식 `UseConstant`을 보여 줍니다.
