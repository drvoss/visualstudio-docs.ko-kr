...
어셈블리가 **ResX**기반 리소스를 포함 하지만 <xref:System.Resources.NeutralResourcesLanguageAttribute?displayProperty=fullName>이 적용 되지 않습니다.

## <a name="rule-description"></a>규칙 설명

@No__t-0 특성은 리소스 관리자에 게 응용 프로그램의 기본 문화권을 알립니다. 기본 문화권의 리소스가 앱의 주 어셈블리에 포함 되 고 <xref:System.Resources.ResourceManager>이 기본 문화권과 같은 문화권에 속하는 리소스를 검색 해야 하는 경우 <xref:System.Resources.ResourceManager>은 검색 대신 주 어셈블리에 있는 리소스를 자동으로 사용 합니다. 위성 어셈블리의 경우입니다. 이 일반적인 어셈블리 프로브 무시, 첫 번째 리소스를 로드 하 고 작업 집합을 줄일 수에 대 한 조회 성능이 향상 됩니다.

> [!TIP]
> @No__t-1에서 리소스 파일을 검색 하는 데 사용 하는 프로세스에 대 한 [리소스 패키징 및 배포](/dotnet/framework/resources/packaging-and-deploying-resources-in-desktop-apps) 를 참조 하세요.
