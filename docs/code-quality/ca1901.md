...
- 반환 값 또는 매개 변수는 고정 크기 정수로 입력 해야 하는 경우 `IntPtr`으로 형식화 됩니다.

## <a name="how-to-fix-violations"></a>위반 문제를 해결하는 방법
@No__t-0 또는 `UIntPtr`을 사용 하 여 `Int32` 또는 `UInt32` 대신 핸들을 나타내면이 위반 문제를 해결할 수 있습니다.
...
이 예제에서 `nIconIndex` 매개 변수는 32 비트 플랫폼의 경우 4 바이트이 고 64 비트 플랫폼에서는 8 바이트의 `IntPtr`로 선언 됩니다. 다음 관리 되지 않는 선언에서-0은 모든 플랫폼에서 부호 없는 4 바이트 정수 @no__t 확인할 수 있습니다.
