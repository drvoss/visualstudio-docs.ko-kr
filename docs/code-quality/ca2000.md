...
@No__t-0 형식의 로컬 개체가 만들어지지만 개체에 대 한 모든 참조가 범위를 벗어난 후에는 개체가 삭제 되지 않습니다.
...
이 규칙 위반 문제를 해결 하려면 개체에 대 한 모든 참조가 범위를 초과 하기 전에 개체에서 <xref:System.IDisposable.Dispose%2A>을 호출 합니다.

[@No__t-1 문](/dotnet/csharp/language-reference/keywords/using-statement) (Visual Basic의[`Using`](/dotnet/visual-basic/language-reference/statements/using-statement) )을 사용 하 여 <xref:System.IDisposable>를 구현 하는 개체를 래핑할 수 있습니다. 이러한 방식으로 래핑된 개체는 `using` 블록의 끝에서 자동으로 삭제 됩니다. 그러나 다음과 같은 경우에는 `using` 문으로 처리 하거나 처리할 수 없습니다.

- 삭제 가능한 개체를 반환 하려면 `using` 블록의 외부 `try/finally` 블록에 개체를 생성 해야 합니다.

- @No__t-0 문의 생성자에서 삭제 가능한 개체의 멤버를 초기화 하지 마십시오.

- 한 예외 처리기로 보호 되는 생성자가 [`using` 문의 획득 부분](/dotnet/csharp/language-reference/language-specification/statements#the-using-statement)에 중첩 된 경우 외부 생성자에서 오류가 발생 하면 중첩 된 생성자에서 만든 개체가 닫히지 않습니다. 다음 예제에서는 <xref:System.IO.StreamReader> 생성자에서 오류가 발생 하 여 @no__t 1 개체가 닫히지 않습니다. CA2000는이 경우 규칙 위반을 플래그 합니다.
...
- @No__t-1과 같이 `Dispose`을 호출 하는 개체에서 메서드를 호출 했습니다.
- 경고를 발생 시킨 메서드는 개체를 래핑하는 @no__t 0 개체를 반환 합니다.
...
- [CA2202: 개체를 여러 번 삭제 하지 않습니다. @ no__t-0
...
@No__t-0은 `try` 블록에서 생성 되 고 열리며 다른 모든 필수 작업은 동일한 `try` 블록에서 수행 됩니다. @No__t-0 블록의 끝에서 열린 포트는 반환 되는 `port` 개체에 할당 되 고 `tempPort` 개체는 `null`으로 설정 됩니다.

@No__t-0 블록은 `tempPort`의 값을 확인 합니다. Null이 아닌 경우 메서드의 작업이 실패 하 고 `tempPort`이 닫혀 모든 리소스가 해제 됩니다. 반환 된 포트 개체에는 메서드의 작업이 성공한 경우 열린 SerialPort 개체가 포함 되 고, 작업이 실패 한 경우에는 null이 됩니다.
