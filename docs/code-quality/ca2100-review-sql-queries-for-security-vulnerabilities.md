...
메서드는 문자열 인수 <xref:System.Data.IDbCommand.CommandText%2A?displayProperty=fullName> 에서 메서드로 만들어진 문자열을 사용 하 여 속성을 설정 합니다.
...
다음 .net 형식은 속성을 <xref:System.Data.IDbCommand.CommandText%2A> 구현 하거나 문자열 인수를 사용 하 여 속성을 설정 하는 생성자를 제공 합니다.
...
다음 예제에서는 매개 변수가 있는 명령 `UnsafeQuery`문자열을 사용 하 여 규칙을 충족 하 `SaferQuery`는, 및 메서드를 위반 하는 메서드를 보여 줍니다.
