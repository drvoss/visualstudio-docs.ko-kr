...
## <a name="cause"></a>원인
<xref:System.Runtime.CompilerServices.RuntimeCompatibilityAttribute>로 표시 되지 않았거나 `RuntimeCompatibility(WrapNonExceptionThrows = false)`로 표시 되어 있고 <xref:System.Exception?displayProperty=fullName>를 처리 하는 catch 블록을 포함 하는 어셈블리의 멤버 이며 바로 다음 일반 catch 블록을 포함 하지 않습니다. 이 규칙은 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 어셈블리를 무시 합니다.
