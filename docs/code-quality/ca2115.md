...
이 규칙 위반 문제를 해결 하려면 <xref:System.GC.KeepAlive%2A>에 대 한 호출을 메서드에 추가 하 여 현재 인스턴스 (및 C# C++의 `this`)를 인수로 전달 합니다. 가비지 수집에서 개체를 보호 해야 하는 코드의 마지막 줄 뒤에 호출을 배치 합니다. @No__t를 호출 하는 즉시 개체는 관리 되는 참조가 없다는 가정 하에 가비지 수집 준비가 된 것으로 간주 됩니다.
