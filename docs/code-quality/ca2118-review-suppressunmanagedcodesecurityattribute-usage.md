...
Public 또는 protected 형식 또는 멤버 <xref:System.Security.SuppressUnmanagedCodeSecurityAttribute?displayProperty=fullName> 에 특성이 있습니다.
...
다음 예제에서 메서드는 `DoWork` 플랫폼 호출 메서드에 `FormatHardDisk`공개적으로 액세스할 수 있는 코드 경로를 제공 합니다.

[!code-csharp[FxCop.Security.PInvokeAndSuppress#1](../code-quality/codesnippet/CSharp/ca2118-review-suppressunmanagedcodesecurityattribute-usage_2.cs)]

## <a name="example-3"></a>예제 3

다음 예제에서는 public 메서드에서 `DoDangerousThing` 위반을 발생 시킵니다. 위반 문제를 해결 하려면 `DoDangerousThing` 를 전용으로 설정 해야 하며, 메서드에 설명 된 `DoWork` 대로 보안 요청으로 보호 되는 공용 메서드를 통해 액세스 해야 합니다.
