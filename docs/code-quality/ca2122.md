...
다음 예제에서는 규칙을 위반 하는 라이브러리 및 라이브러리의 약점을 보여 주는 응용 프로그램을 보여 줍니다. 샘플 라이브러리는 규칙을 위반 하는 두 가지 메서드를 제공 합니다. `EnvironmentSetting` 메서드는 환경 변수에 대 한 무제한 액세스를 위해 링크 요청에 의해 보호 됩니다. `DomainInformation` 메서드는 `EnvironmentSetting`를 호출 하기 전에 호출자의 보안 요구를 요구 하지 않습니다.
