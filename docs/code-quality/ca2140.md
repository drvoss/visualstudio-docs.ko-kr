...
<xref:System.Security.SecurityCriticalAttribute> 특성으로 표시 된 코드 요소는 보안에 중요 합니다. 투명 메서드는 보안에 중요한 요소를 사용할 수 없습니다. 투명 형식이 보안에 중요 한 형식을 사용 하려고 시도 하는 경우 <xref:System.TypeAccessException>, <xref:System.MethodAccessException> 또는 <xref:System.FieldAccessException> 발생 합니다.
