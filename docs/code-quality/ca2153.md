...
이 경고는 모든 예외를 catch 하는 일반 처리기로 CSEs를 catch 할 때 트리거됩니다. 예를 들어 `catch (System.Exception e)` 또는 `catch` (예외 없음 매개 변수 포함)을 catch 합니다.
...
@No__t-0 특성을 제거 하면 메서드에서 손상 된 상태 예외를 처리 하지 않습니다.
