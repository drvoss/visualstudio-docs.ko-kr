...
- .NET Framework 2.0의 경우 `/keyfile` 또는 `/keycontainer` 컴파일러 옵션 [/keyfile (어셈블리에 서명할 키 또는 키 쌍 지정](/cpp/build/reference/keyfile-specify-key-or-key-pair-to-sign-an-assembly) ) 또는 [/KEYCONTAINER (어셈블리에 서명할 키 컨테이너 지정](/cpp/build/reference/keycontainer-specify-a-key-container-to-sign-an-assembly) ) 링커 옵션 C++중 하나를 사용 합니다.
