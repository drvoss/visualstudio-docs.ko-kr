...
## <a name="cause"></a>원인
Public 형식은 <xref:System.Object.Equals%2A?displayProperty=fullName>을 재정의 하지만 <xref:System.Object.GetHashCode%2A?displayProperty=fullName>은 재정의 하지 않습니다.

## <a name="rule-description"></a>규칙 설명
 <xref:System.Object.GetHashCode%2A>은 해시 알고리즘 및 해시 테이블과 같은 데이터 구조에 적합 한 현재 인스턴스를 기반으로 값을 반환 합니다. 동일한 형식이 고 동일한 두 개체가 동일한 해시 코드를 반환 하 여 다음 형식의 인스턴스가 제대로 작동 하는지 확인 해야 합니다.
...
이 규칙 위반 문제를 해결 하려면-0 @no__t의 구현을 제공 합니다. 동일한 형식의 개체 쌍에 대해 <xref:System.Object.Equals%2A>의 구현이 쌍의 `true`을 반환 하는 경우 구현이 같은 값을 반환 하는지 확인 해야 합니다.
