...
이 예제의 메서드가 전달 <xref:System.Int32.MinValue?displayProperty=fullName>되 면 연산이 언더플로 됩니다. 이렇게 하면 결과가 가장 중요 한 비트를 삭제 합니다. 다음 코드에서는이를 수행 하는 방법을 보여 줍니다.
...
다음 예제에서는 확인 된 블록에서 작업을 래핑하여 이전 위반을 수정 합니다. 작업에서 오버플로가 <xref:System.OverflowException?displayProperty=fullName> 발생 하면이 throw 됩니다.

선택 된 블록은에서 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]지원 되지 않습니다.
