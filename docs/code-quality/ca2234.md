...
다음 예제에서는 <xref:System.Uri> 오버 로드를 올바르게 호출 하는 규칙 및 메서드 (`SaferWay`)를 위반 하는 `ErrorProne` 메서드를 보여 줍니다.
