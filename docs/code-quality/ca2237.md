...
공용 언어 런타임에서 serializable로 인식 하려면 형식이 <xref:System.Runtime.Serialization.ISerializable> 인터페이스의 구현을 통해 사용자 지정 serialization 루틴을 사용 하는 경우에도 <xref:System.SerializableAttribute> 특성으로 표시 해야 합니다.
