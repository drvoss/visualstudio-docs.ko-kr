...
## <a name="cause"></a>원인
형식에 <xref:System.Runtime.Serialization.OptionalFieldAttribute?displayProperty=fullName> 특성으로 표시 되는 필드가 있으며, 형식에서 역직렬화 이벤트 처리 메서드를 제공 하지 않습니다.
