...
*DTD(문서 종류 정의)* 는 XML 파서가  [W3C(World Wide Web Consortium) XML(Extensible Markup Language) 1.0](http://www.w3.org/TR/2008/REC-xml-20081126/)에 정의된 대로 문서의 유효성을 확인할 수 있는 두 가지 방법 중 하나입니다. 이 규칙은 신뢰할 수 없는 데이터가 허용 되는 속성 및 인스턴스를 검색 하 여 잠재적인 [정보 공개](/dotnet/framework/wcf/feature-details/information-disclosure) 위협 또는 [Dos (서비스 거부)](/dotnet/framework/wcf/feature-details/denial-of-service) 공격에 대해 개발자에 게 경고할 수 있습니다. 다음 경우에 이 규칙이 트리거됩니다.
...
- 신뢰할 수 없는 입력은 <xref:System.Xml.XmlResolver> 대신을 <xref:System.Xml.XmlSecureResolver>사용 하 여 처리 됩니다.

- 메서드 <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> 는 안전 <xref:System.Xml.XmlReaderSettings> 하지 않은 인스턴스를 사용 하거나 인스턴스 없이 호출 됩니다.
...
- XmlTextReader에서 액세스할 수 있는 리소스를 제한 하려면를사용합니다.<xref:System.Xml.XmlSecureResolver>
...
- 속성을 <xref:System.Xml.XmlReaderSettings.ProhibitDtd%2A> **true**로 설정 하 여 신뢰할 수 없는 소스를 처리 하는 경우 DTD 처리를 사용 하지 않도록 설정 합니다.

- XmlTextReader 클래스에는 완전 신뢰 상속 요청이 있습니다.

**.NET 4 이상**

- 속성을 <xref:System.Xml.XmlReaderSettings.DtdProcessing%2A?displayProperty=nameWithType> **금지** 또는 **무시**로 설정 하 여 신뢰할 수 없는 소스를 처리 하는 경우 DtdProcessing를 사용 하지 않도록 합니다.
