...
- <xref:System.Xml.XmlReaderSettings.DtdProcessing%2A> 속성은 Parse로 설정 됩니다.

- 신뢰할 수 없는 입력은-1 @no__t 대신 <xref:System.Xml.XmlResolver>을 사용 하 여 처리 됩니다.

- @No__t-0 메서드는 안전 하지 않은 @no__t 1 인스턴스를 사용 하거나 인스턴스를 사용 하지 않고 호출 됩니다.
...
- @No__t-0을 사용 하 여 XmlTextReader에서 액세스할 수 있는 리소스를 제한 합니다.

- <xref:System.Xml.XmlReader> 속성을 <xref:System.Xml.XmlResolver> null **로 설정하여**가 외부 리소스를 열 수 없도록 합니다.

- @No__t-0 속성이 신뢰할 수 있는 소스에서 할당 되었는지 확인 합니다.

**.NET 3.5 및 이전 버전**

- @No__t-0 속성을 **true**로 설정 하 여 신뢰할 수 없는 소스를 처리 하는 경우 DTD 처리를 사용 하지 않도록 설정 합니다.

- XmlTextReader 클래스에는 완전 신뢰 상속 요청이 있습니다.

**.NET 4 이상**

- @No__t-0 속성을 **금지** 또는 **무시**로 설정 하 여 신뢰할 수 없는 소스를 처리 하는 경우 DtdProcessing를 사용 하지 않도록 합니다.
