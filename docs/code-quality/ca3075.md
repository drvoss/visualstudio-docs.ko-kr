...
- @No__t_0 메서드는 안전 하지 않은 <xref:System.Xml.XmlReaderSettings> 인스턴스를 사용 하거나 인스턴스 없이 호출 됩니다.
...
- @No__t_0 속성이 신뢰할 수 있는 소스에서 할당 되었는지 확인 합니다.
...
- @No__t_0 속성을 **금지** 또는 **무시**로 설정 하 여 신뢰할 수 없는 소스를 처리 하는 경우 DtdProcessing를 사용 하지 않도록 합니다.
...
> [!NOTE]
> @No__t_0는 <xref:System.Xml.XmlReader> 인스턴스를 만드는 데 권장 되는 방법 이지만 <xref:System.Xml.XmlTextReader>와의 동작 차이가 있습니다. @No__t_1에서 <xref:System.Xml.XmlReader>는 `\r\n` `\n`를 정규화 하 고 <xref:System.Xml.XmlTextReader> 시퀀스를 유지 합니다.
