...
공격 노출 영역을 노출 하지 않도록 하기 위해이 규칙은 XslCompiledTransform 될 때마다 트리거됩니다.<xref:System.Xml.Xsl.XslCompiledTransform.Load%2A> 악의적인 스크립트 처리를 허용 <xref:System.Xml.Xsl.XsltSettings> 하 <xref:System.Xml.XmlResolver>는 및의 안전 하지 않은 조합 인스턴스를 수신 합니다.
