...
- 응용 프로그램이 .NET Framework v 4.6.2 이전 버전을 대상으로 하 고 .NET Framework v 4.7 이상에서 실행 되는 경우 스위치의 값을 `false`으로 설정 합니다.
