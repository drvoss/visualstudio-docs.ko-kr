...
사용자 지정 규칙 집합을 만들려면 **규칙 집합 편집기**에서 기본 제공 규칙 집합을 열 수 있습니다. 여기에서 특정 규칙을 추가 하거나 제거할 수 있으며, 규칙이 위반 될 때 발생 하는 동작을 변경할 수 있습니다. 예를 들어 경고 또는 오류를 표시&mdash;.
...
::: moniker-end
   ...
   - **\<찾아보기 >** 를 선택 하 여 목록에 없는 기존 규칙 집합을 지정 합니다.
...
3. \<선택 하 여 **이 규칙 집합 실행** **> 여러 규칙 집합** 을 선택 합니다.

::: moniker-end

::: moniker range=">=vs-2019"

3. \<선택 하 여 **활성 규칙**에서 **> 여러 규칙 집합을 선택** 합니다.
...
- 동일한 규칙이 심각도가 *같은* 규칙 집합에서 두 번 이상 나열 되는 경우 **오류 목록**에 다음 경고가 표시 될 수 있습니다.
   ...
   **CA0063: 규칙 집합 파일 '\[]. 규칙 집합 ' 또는 종속 규칙 집합 파일 중 하나를 로드 하지 못했습니다. 파일이 규칙 집합 스키마를 따르지 않습니다.**
