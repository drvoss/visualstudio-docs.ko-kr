...
> [!NOTE]
> SELECT 명령에 할당 되는 저장 프로시저는 마법사의 다음 단계에서 이름을 지정 하는 TableAdapter의 메서드를 호출 하 여 실행 합니다. 기본 메서드는 `Fill`이므로 일반적으로 SELECT 프로시저를 실행 하는 데 사용 되는 코드는 `TableAdapter.Fill(tableName)`됩니다. `Fill`에서 기본 이름을 변경 하는 경우 사용자가 할당 한 이름으로 `Fill`를 대체 하 고 "TableAdapter"를 TableAdapter의 실제 이름 (예: `CustomersTableAdapter`)으로 바꿉니다.
