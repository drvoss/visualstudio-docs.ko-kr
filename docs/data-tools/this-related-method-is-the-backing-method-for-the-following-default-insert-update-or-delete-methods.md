...
선택한 `DataContext` 메서드는 현재 **O/R 디자이너**의 엔터티 클래스 `Update`중 하나 `Delete` 에 대 한 `Insert`, 또는 메서드 중 하나로 사용 됩니다. 선택한 메서드를 삭제 하면이 메서드를 사용 하는 엔터티 클래스가 업데이트 중에 삽입, 업데이트 또는 삭제를 수행 하는 기본 런타임 동작으로 되돌아갑니다.
