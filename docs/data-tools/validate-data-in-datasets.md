...
- <xref:System.Data.DataColumn.MaxLength%2A>, <xref:System.Data.DataColumn.AllowDBNull%2A>및 <xref:System.Data.DataColumn.Unique%2A>과 같은 <xref:System.Data.DataColumn> 개체의 속성을 설정 합니다.

레코드에서 변경이 발생 하는 경우 <xref:System.Data.DataTable> 개체에 의해 발생 하는 몇 가지 이벤트는 다음과 같습니다.

- <xref:System.Data.DataTable.ColumnChanging> 및 <xref:System.Data.DataTable.ColumnChanged> 이벤트는 개별 열을 변경할 때마다 발생 합니다. <xref:System.Data.DataTable.ColumnChanging> 이벤트는 특정 열의 변경 내용에 대 한 유효성을 검사 하려는 경우에 유용 합니다. 제안 된 변경에 대 한 정보는 이벤트와 함께 인수로 전달 됩니다.
- <xref:System.Data.DataTable.RowChanging> 및 <xref:System.Data.DataTable.RowChanged> 이벤트는 행을 변경한 후에 발생 합니다. <xref:System.Data.DataTable.RowChanging> 이벤트는 더 일반적입니다. 이는 행에서 변경 내용이 발생 하지만 변경 된 열을 알 수 없음을 나타냅니다.
...
<xref:System.Data.DataTable.ColumnChanging>, <xref:System.Data.DataTable.RowChanging>및 <xref:System.Data.DataTable.RowDeleting> 이벤트는 업데이트 프로세스 중에 발생 합니다. 이러한 이벤트를 사용 하 여 데이터의 유효성을 검사 하거나 다른 유형의 처리를 수행할 수 있습니다. 이러한 이벤트 중에 업데이트가 진행 되 고 있으므로 예외를 throw 하 여 업데이트를 취소할 수 있습니다. 이렇게 하면 업데이트를 완료할 수 없습니다.
