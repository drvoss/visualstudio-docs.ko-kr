...
응용 프로그램에 유효성 검사를 추가 하는 가장 좋은 장소는 데이터 집합의 partial 클래스 파일에 있습니다. @No__t_0 또는 [!INCLUDE[csprcs](../data-tools/includes/csprcs_md.md)]에서 **데이터 세트 디자이너** 을 열고 유효성 검사를 만들 열 또는 테이블을 두 번 클릭 합니다. 이 작업은 <xref:System.Data.DataTable.ColumnChanging> 또는 <xref:System.Data.DataTable.RowChanging> 이벤트 처리기를 자동으로 만듭니다.
...
- @No__t_0 및 <xref:System.Data.DataTable.ColumnChanged> 이벤트는 개별 열을 변경할 때마다 발생 합니다. @No__t_0 이벤트는 특정 열의 변경 내용에 대 한 유효성을 검사 하려는 경우에 유용 합니다. 제안 된 변경에 대 한 정보는 이벤트와 함께 인수로 전달 됩니다.
- @No__t_0 및 <xref:System.Data.DataTable.RowChanged> 이벤트는 행을 변경한 후에 발생 합니다. @No__t_0 이벤트는 더 일반적입니다. 이는 행에서 변경 내용이 발생 하지만 변경 된 열을 알 수 없음을 나타냅니다.
...
@No__t_0 이벤트 중에 유효성 검사를 수행할 수도 있습니다.
