...
할당 후크 함수에 대한 매우 중요한 제한 사항은 `_CRT_BLOCK` 블록을 명시적으로 무시 해야 한다는 것입니다. 이러한 블록은 내부 메모리를 할당하는 C 런타임 라이브러리 함수를 호출 하는 경우 C 런타임 라이브러리 함수에 의해 내부적으로 생성된 메모리 할당입니다. 할당 후크 함수의 시작 부분에 다음 코드를 포함하여 `_CRT_BLOCK` 블록을 무시할 수 있습니다.
