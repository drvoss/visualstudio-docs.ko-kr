...
ms.locfileid: "71814367"
---
# <a name="use-breakpoints-in-the-visual-studio-debugger"></a>Visual Studio 디버거에서 중단점 사용
중단점은 개발자의 도구 상자에서 가장 중요 한 디버깅 기술 중 하나입니다. 디버거 실행을 일시 중지 하려는 중단점을 설정 합니다. 예를 들어 코드 변수의 상태를 확인 하거나 특정 중단점에서 호출 스택을 확인할 수 있습니다. 코드를 처음으로 디버그하려고 하는 경우 이 문서를 계속 진행하기 전에 먼저 [완전 초보자를 위한 디버깅](../debugger/debugging-absolute-beginners.md)을 참조하는 것이 좋습니다.
...
## <a name="BKMK_Overview"></a>소스 코드에 중단점 설정
 ...
 소스 코드에서 중단점을 설정 하려면 코드 줄 옆에 있는 맨 왼쪽 여백을 클릭 합니다. 줄을 선택 하 고 **F9**키를 누르거나, **디버그** > **중단점 설정/해제**를 선택 하거나, 마우스 오른쪽 단추를 클릭 하 고 **중단점 @no__t** **중단점 삽입**을 선택할 수도 있습니다. 중단점이 왼쪽 여백에 빨간색 점으로 표시 됩니다.

코드 C# 에서 중단점 및 현재 실행 줄은 자동으로 강조 표시 됩니다. 코드 C++ 의 경우 **도구** (또는 **디버그**) > **옵션**을 선택 하 여 중단점 및 현재 줄의 강조 표시를 설정할 수 있습니다 **. 4 @no__t**@no__t **문 (C++ 에만 해당)** .

 ![중단점](../debugger/media/basicbreakpoint.png "기본 중단점") 설정

 디버그할 때 해당 줄의 코드가 실행 되기 전에 중단점에서 실행이 일시 중지 됩니다. 중단점 기호에는 노란색 화살표가 표시 됩니다.

 다음 예제의 중단점에서 `testInt`의 값은 여전히 1입니다.

 ![중단점 실행이 중지]된(../debugger/media/breakpointexecution.png "중단점 실행")

 디버거가 중단점에서 중지 되 면 변수 값 및 호출 스택을 포함 하 여 응용 프로그램의 현재 상태를 확인할 수 있습니다. 호출 스택에 대 한 자세한 내용은 [How to: 호출 스택 창 @ no__t-0을 사용 합니다.

- 중단점은 설정/해제입니다. 이를 클릭 하거나 **f9**키를 누르거나, **디버그** > **중단점 설정/해제** 를 사용 하 여 삭제 하거나 다시 삽입할 수 있습니다.
...
호출 스택에 대 한 자세한 내용은 [How to: 호출 스택 창 @ no__t-0을 사용 합니다.
...
2. **디스어셈블리** 창에서 중단 하려는 명령의 왼쪽 여백을 클릭 합니다. 이를 선택 하 고 **F9**키를 누르거나 마우스 오른쪽 단추를 클릭 하 고 **중단점 @no__t-** 2**중단점 삽입**을 선택할 수도 있습니다.
...
1. **디버그** > **새 중단점** > **함수 중단점**을 선택 하거나 **Alt**+**F9** > **ctrl**0**B**를 누릅니다.
...
### <a name="set-a-function-breakpoint-using-a-memory-address-native-c-only"></a>메모리 주소를 사용 하 여 함수 중단점 설정 ( C++ 네이티브 전용)
 개체의 주소를 사용 하 여 클래스의 특정 인스턴스에서 호출 되는 메서드에 함수 중단점을 설정할 수 있습니다.  예를 들어 `my_class` 형식의 주소 지정 가능한 개체가 지정 된 경우 인스턴스가 호출 하는 @no__t 1 메서드에 함수 중단점을 설정할 수 있습니다.

1. 클래스의 인스턴스가 인스턴스화된 후 어딘가에 중단점을 설정 합니다.

2. 인스턴스의 주소를 찾습니다 (예: `0xcccccccc`).

3. **디버그** > **새 중단점** > **함수 중단점**을 선택 하거나 **Alt**+**F9** > **ctrl**0**B**를 누릅니다.
...
1. C++ 프로젝트에서 디버깅을 시작 하 고 중단점에 도달할 때까지 기다립니다. **디버그** 메뉴에서 **새 중단점** > **데이터 중단점** 을 선택 합니다.
    ...
    **중단점** 창에서 **새** > **데이터 중단점** 을 선택 하거나 **자동**, **조사식**또는 **지역** 창의 항목을 마우스 오른쪽 단추로 클릭 하 고 상황에 맞는 메뉴에서 **값이 변경 되 면 중단** 을 선택할 수도 있습니다.
...
**중단점** 창을 열려면 **디버그** > **Windows** > **중단점**을 선택 하거나 **alt**+**F9** 또는 **Ctrl**0**alt**2**B**를 누릅니다.
...
1. 중단점 기호를 마우스 오른쪽 단추로 클릭 하 고 **조건**을 선택 합니다. 또는 중단점 기호 위로 마우스를 이동 하 고 **설정** 아이콘을 선택한 다음 **중단점 설정** 창에서 **조건** 을 선택 합니다.
   ...
   **중단점 창에서** 중단점을 마우스 오른쪽 단추로 클릭 하 고 **설정**을 선택한 다음 **조건**을 선택 하 여 조건을 설정할 수도 있습니다.
   ...
   ![중단점 설정]간의(../debugger/media/breakpointsettings.png "pointsettings")

2. 드롭다운에서 **조건 식**, **적중 횟수**또는 **필터**를 선택 하 고 값을 적절 하 게 설정 합니다.

3. **닫기** 를 선택 하거나 **ctrl**+**enter** 를 눌러 **중단점 설정** 창을 닫습니다. 또는 **중단점** 창에서 **확인** 을 선택 하 여 대화 상자를 닫습니다.

조건이 설정 된 중단점은 소스 코드 및 **중단점** 창에 **+** 기호와 함께 표시 됩니다.

<a name="BKMK_Specify_a_breakpoint_condition_using_a_code_expression"></a>
### <a name="conditional-expression"></a>조건식

**조건식**을 선택 하는 경우 다음 두 조건 중에서 선택할 수 있습니다. **가 true** 이거나 **변경 된 경우** 식이 충족 될 때 중단 하거나 식의 값이 변경 될 때 중단 하도록 **변경** 된 경우에 **는 true** 를 선택 합니다.

 다음 예제에서는 @no__t 값이 **4**인 경우에만 중단점이 적중 됩니다.

 ![중단점 조건은 true](../debugger/media/breakpointconditionistrue.png "중단점") 입니다.

 다음 예에서는 `testInt`의 값이 변경 되는 경우에만 중단점이 적중 됩니다.

 변경(../debugger/media/breakpointwhenchanged.png "시 중단점") ![변경 시 중단점]
...
2. 디버깅을 시작 하 고, 실행이 중단점에서 일시 중지 되 면 **디버그** > **Windows** > **로컬** 또는 **Alt**+**4** 를 선택 하 여 **지역** 창을 엽니다.
...
4. **조건부 식** 필드에 개체 ID를 사용합니다. 예를 들어 변수 `item`이 컬렉션에 추가할 개체 이면 **true** 를 선택 하 고 **item = = $ \<n >** 을 입력 합니다. 여기서 \<n >은 개체 ID 번호입니다.
