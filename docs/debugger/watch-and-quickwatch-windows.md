...
1. 다음 코드를 입력합니다. 그리고 **조사식** 창에서 `list.Count`속성에 조사식을 설정합니다.
...
1. 디버깅을 시작합니다. **조사식** 창에 다음 메시지가 표시됩니다:
...
때로는 특정 개체의 동작을 관찰하려는 경우가 있습니다. 예를 들어, 해당 변수가 범위를 벗어난 후 로컬 변수가 참조하는 개체를 추적하고자 할 수 있습니다. C# 및 Visual Basic에서는 참조 형식의 특정 인스턴스에 대한 개체 ID를 만들고 **조사식** 창 및 중단점 조건에서 사용할 수 있습니다. 개체 ID는 CLR(공용 언어 런타임) 디버깅 서비스에 의해 생성되고 개체와 연결됩니다.
...
1. 중단점에서 실행이 일시 정지되면 **디버그** ****창** ****로컬**을 선택하여 **로컬** 창을 엽니다.

1. **로컬** 창에서 `Person` 변수를 마우스 오른쪽 단추로 클릭하고 **개체 ID 만들기**를 선택하세요.

   **로컬** 창에 달러 기호( **$** )와 숫자가 표시되어야 합니다. 이는 개체 ID입니다.

1. 개체 ID를 마우스 오른쪽 단추로 클릭하고 **조사식 추가**를 선택하여 **조사식** 창에 개체 ID를 추가합니다.

1. `DoSomething()` 메서드에 또 다른 중단점을 설정합니다.
...
>- **동적 뷰**의 멤버를 계산하면 [부작용](https://en.wikipedia.org/wiki/Side_effect_\(computer_science\))이 발생할 수 있습니다.

**객체를 동적 객체로 캐스팅하는 새로운 조사식 변수를 삽입하려면:**

1. **동적 뷰**의 하위를 마우스 오른쪽 단추로 클릭합니다.
1. **조사식 추가**를 선택합니다. `object.name`은 `((dynamic) object).name`이 되고 새로운 **조사식** 창에 나타납니다.
...
1. 변수를 사용하여 표현식을 평가하려면 **표현식** 상자에 `a + b`와 같은 표현식을 입력하고 **재평가**를 선택하세요.

   ![조사식](../debugger/media/quickwatchexpression.png "조사식")

1. 변수 또는 식을 **QuickWatch**에서 **조사식** 창에 추가하려면 **조사식 추가**를 선택하세요.

1. 7\.**QuickWatch** 창을 닫으려면 **닫기**를 선택하세요. (**QuickWatch**는 모달 대화 상자이므로 열려 있는 한 계속 디버깅할 수 없습니다.)
