...
4. 값을 새로 고치려면 새로 고침 아이콘을 선택하거나 스페이스바를 누릅니다. 디버거가 식을 다시 계산합니다.
...
2. 디버깅을 시작합니다. 실행이 중단점에서 일시 중지합니다.

3. 코드에서 변수 `a`를 선택합니다.

4. **디버그> QuickWatch**를 선택하거나 **Shift + F9**를 누르거나 마우스 오른쪽 단추를 클릭하고 **QuickWatch**를 선택하세요.
...
6. 변수 또는 식을 **QuickWatch**에서 **조사식** 창에 추가하려면 **조사식 추가**를 선택하세요.
