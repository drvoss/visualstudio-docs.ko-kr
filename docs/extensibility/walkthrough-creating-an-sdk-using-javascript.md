...
이 연습에서는 JavaScript를 이용해 간단한 수학 SDK를 만들어 Visual Studio 확장 (VSIX)로 사용하는 방법을 설명 합니다.  이 연습에서는 다음과 같은 파트로 구분 됩니다.
...
- [SDK를 사용 하는 샘플 앱을 만들려면](../extensibility/walkthrough-creating-an-sdk-using-javascript.md#createSampleApp)
  ...
  JavaScript는 클래스 라이브러리 프로젝트 형식이 없습니다. 이 연습에서는 샘플 *arithmetic.js* 파일이 VSIX 프로젝트에 직접 생성됩니다. 실전에서는 VSIX 프로젝트에 삽입하기 전에 JavaScript 및 CSS 파일을 먼저 Windows 스토어 앱-예로,**비어 있는 앱**-으로 빌드 및 테스트 하는 것을 권장합니다.
...
2. 템플릿 범주 목록에서 아래 **Visual C#** 를 선택 **확장성**를 선택한 후는 **VSIX 프로젝트** 템플릿을 선택합니다.

3. **이름** 텍스트 상자에서, `SimpleMathVSIX` 선택 후 **확인** 버튼 클릭합니다.

4. **Visual Studio 패키지 마법사**가 나타나는 경우, **시작** 페이지에서 **다음** 단추를 클릭 한 후 **7 페이지 중 1 페이지**에서 **완료** 버튼을 선택합니다.  

     비록 **매니페스트 디자이너**가 열리지만, 이 연습에서는 매니페스트 파일을 직접적으로 간단하게 수정 하겠습니다.
