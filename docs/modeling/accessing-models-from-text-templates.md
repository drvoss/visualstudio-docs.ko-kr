...
- 액세스 하려는 DSL에 대 한 지시문 프로세서를 지정 합니다. 그러면 텍스트 템플릿의 코드에서 해당 도메인 클래스, 속성 및 관계를 사용할 수 있도록 DSL에 대 한 어셈블리가 로드 됩니다. 또한 지정한 모델 파일도 로드 합니다.
  ...
  다음 예제와 유사한 `.tt` 파일은 DSL 최소 언어 템플릿에서 새 Visual Studio 솔루션을 만들 때 디버깅 프로젝트에서 만들어집니다.
...
- 코드 조각이 작성 된 언어가 인 C#경우에도 모든 종류의 텍스트를 생성할 수 있습니다. 또는 `template` 지시문에 `language="VB"` 속성을 추가 하 여 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]에 코드를 작성할 수 있습니다.

- 템플릿을 디버깅 하려면 `template` 지시문에 `debug="true"`를 추가 합니다. 예외가 발생 하는 경우 템플릿은 Visual Studio의 다른 인스턴스에서 열립니다. 코드의 특정 지점에서 디버거를 중단 하려면 문을 삽입 `System.Diagnostics.Debugger.Break();`
...
## <a name="loading-models-dynamically"></a>동적으로 모델 로드
 ...
 일반적이 지 않은 시나리오에서는 파일 이름만 있고 현재 Visual Studio 프로젝트에 없을 수 있는 모델 파일을 열어야 할 수 있습니다. 이 경우 [How에 설명 된 기술을 사용 하 여 파일을 열 수 있습니다. 프로그램 코드 ](../modeling/how-to-open-a-model-from-file-in-program-code.md)의 파일에서 모델을 엽니다.
