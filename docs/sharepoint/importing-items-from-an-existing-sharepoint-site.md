...
> [!NOTE]
> 재사용 가능한 워크플로를 가져오려면 재사용 가능한 워크플로 가져오기 프로젝트 템플릿을 사용합니다. [재사용 가능한 워크플로 가져오기에 대 한 지침을](../sharepoint/guidelines-for-importing-reusable-workflows.md)[!INCLUDE[crdefault](../sharepoint/includes/crdefault-md.md)] 합니다.
...
- 사이트 정의
  ...
  [!INCLUDE[wss_14_short](../sharepoint/includes/wss-14-short-md.md)] 또는 [!INCLUDE[moss_14_short](../sharepoint/includes/moss-14-short-md.md)]에서 솔루션을 내보내는 경우 이러한 항목은 *.wsp* 파일에서 자동으로 제외 됩니다. 그러나 지원 되지 않는 도구에서 생성 된 다른 *.wsp* 파일에는 이러한 항목이 포함 될 수 있습니다. 자세한 내용은 이 항목의 앞부분에 나오는 "지원되는 SharePoint 솔루션"을 참조하세요.

## <a name="what-happens-when-you-import-a-solution"></a>솔루션을 가져올 때 수행 되는 작업
 SharePoint 솔루션 패키지 가져오기 템플릿을 사용 하 여 솔루션을 가져올 때 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]은 .wsp 파일의 모든 내용을 복사 하 고 가져온 요소와 해당 파일 간의 연결 및 참조의 수를 조정 하 고 유지 하려고 *합니다.* 가능한.
...
### <a name="handle-special-cases"></a>특수 한 경우 처리
 ...
 예를 들어 목록 정의 ExpenseForms를 가져오는 경우 해당 이름을 가진 목록 정의가 해당 *Elements* 및 *Schema .xml* 파일과 함께 **솔루션 탐색기** 의 **목록** 정의 폴더 아래에 나타납니다. 그러나 관련 ASPX 및 HTML 양식은 **기타 가져온 파일** 폴더의 **ExpenseForms** 폴더 아래 배치될 수 있습니다. 가져오기를 완료하려면 **솔루션 탐색기** 에서 이러한 파일을 ExpenseForms 목록 정의로 이동하고 각 파일의 **DeploymentType** 속성을 **NoDeployment** 에서 **ElementFile**로 변경합니다.

 이벤트 수신기를 가져올 때 *Elements* 파일은 올바른 위치에 복사 되지만 솔루션과 함께 배포 되도록 어셈블리를 솔루션 패키지에 수동으로 포함 해야 합니다. 이 작업을 수행 하는 방법 [!INCLUDE[crabout](../sharepoint/includes/crabout-md.md)] [방법: 추가 어셈블리 추가 및 제거](../sharepoint/how-to-add-and-remove-additional-assemblies.md)를 참조 하세요.
...
## <a name="import-fields-and-property-bags"></a>필드 및 속성 모음 가져오기
 여러 필드를 포함 하는 솔루션을 가져오면 모든 개별 필드 정의가 **솔루션 탐색기** **필드**라는 노드 아래의 단일 *Elements .xml* 파일로 병합 됩니다. 마찬가지로 모든 속성 모음 항목은 **propertybags**이라는 노드 아래의 *Elements .xml* 파일로 병합 됩니다.
