...
Visual Studio의 CMake 지원은 Visual Studio 프로젝트 시스템과 관련이 없습니다. 따라서 다른 CMake 환경에서와 마찬가지로 CTest 테스트를 작성 및 구성합니다. `enable_testing()` 명령을 사용하여 테스트를 사용하도록 설정하고 `add_test()` 명령을 사용하여 새 테스트를 추가합니다. CTest에 대한 자세한 내용은 [CMake 설명서](https://gitlab.kitware.com/cmake/community/wikis/doc/ctest/Testing-With-CTest)를 참조하세요. 
