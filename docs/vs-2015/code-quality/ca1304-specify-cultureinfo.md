...
## <a name="rule-description"></a>규칙 설명
 <xref:System.Globalization.CultureInfo> 또는 <xref:System.IFormatProvider?displayProperty=fullName> 개체를 제공 하지 않으면 오버 로드 된 멤버에서 제공 하는 기본값이 모든 로캘에서 원하는 효과를 갖지 않을 수 있습니다. 또한 멤버 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 기본 문화권을 선택 하 고 코드에 맞지 않을 수 있는 가정을 기반으로 서식을 지정 합니다. 시나리오에서 코드가 예상 대로 작동 하도록 하려면 다음 지침에 따라 문화권별 정보를 제공 해야 합니다.
