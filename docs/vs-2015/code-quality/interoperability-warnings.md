...
|[CA1403: 자동 레이아웃 형식은 COM 노출이면 안 됩니다.](../code-quality/ca1403-auto-layout-types-should-not-be-com-visible.md)|COM 노출 값 형식은 Layoutkind.explicit로 설정 된 T e m. StructLayoutAttribute 특성을 사용 하 여 표시 됩니다. 이러한 형식의 레이아웃은 특정 레이아웃이 요구 되는 COM 클라이언트를 중단 하는 [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] 버전 간에 변경 될 수 있습니다.|
|[CA1404: P-Invoke 다음에 바로 GetLastError를 호출합니다.](../code-quality/ca1404-call-getlasterror-immediately-after-p-invoke.md)|GetLastWin32Error 메서드 또는 이와 동등한 [!INCLUDE[TLA2#tla_win32](../includes/tla2sharptla-win32-md.md)] GetLastError 함수를 호출 하 고, 바로 이전 호출은 플랫폼 호출 메서드에 적용 되지 않습니다.|
