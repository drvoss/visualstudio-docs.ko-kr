...
|Index|탭|키워드 입력에서 키워드 목록으로 포커스를 이동합니다.|
|Index|Ctrl+K|지정하는 키워드가 포함된 입력과 지정하는 키워드로 시작하는 입력 간에 표시를 전환합니다.|
