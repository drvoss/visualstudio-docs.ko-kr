...
6. 8\.1 값을 사용 하 여 \<TargetPlatformVersion > 요소가 포함 된 \<PropertyGroup > 요소를 찾습니다. 이 \<PropertyGroup > 요소에 대해 다음 단계를 수행 합니다.

    1. @No__t_0Platform > 요소의 값을 **x 86**으로 설정 합니다.

    2. @No__t_0TargetPlatformIdentifier > 요소를 추가 하 고 해당 값을 **UAP**로 설정 합니다.

    3. @No__t_0TargetPlatformVersion > 요소의 기존 값을 설치한 유니버설 Windows 플랫폼 버전의 값으로 변경 합니다. 또한 \<TargetPlatformMinVersion > 요소를 추가 하 고 동일한 값을 지정 합니다.

    4. @No__t_0MinimumVisualStudioVersion > 요소의 값을 **14**로 변경 합니다.
    ...
    6. @No__t_0EnableDotNetNativeCompatibleProfile > 요소를 추가 하 고 해당 값을 **true**로 설정 합니다.

    7. 유니버설 Windows 앱의 기본 자산 배율은 200입니다. 프로젝트에 200에서 크기가 조정 되지 않은 자산이 포함 된 경우 자산 규모의 값이 PropertyGroup > 요소를이에 \<UapDefaultAssetScale 추가 해야 합니다. [자산 및 배율](https://msdn.microsoft.com/library/jj679352.aspx)에 대해 자세히 알아보세요.

         이제 \<PropertyGroup > 요소가 다음 예제와 같이 표시 됩니다.
...
9. 남아 있는 각 \<PropertyGroup > 요소에 대해 요소에 릴리스 구성이 있는 Condition 특성이 있는지 확인 합니다. 이 경우 \<UseDotNetNativeToolchain > 요소가 포함 되지 않은 경우 하나를 추가 합니다. 다음과 같이 \<UseDotNetNativeToolchain > 요소에 대 한 값을 true로 설정 합니다.
...
11. @No__t_1AppxManifest > 요소를 포함 하는 \<ItemGroup > 요소를 찾습니다. 다음 \<None > 요소를 \<ItemGroup > 요소의 자식으로 추가 합니다.

    ```xml
    <None Include="project.json" />
    ```

12. 로고 .png 파일과 같이 프로젝트에 추가 되는 기타 자산을 포함 하는 \<ItemGroup > 요소 (\<Content Include = "Assets\Logo.scale-100.png"/>)를 찾습니다. 다음 \<Content > 자식 요소를이 \<ItemGroup > 요소에 추가 합니다.
...
13. NuGet 패키지에 대 한 \<Reference > 자식 요소를 포함 하는 \<ItemGroup > 요소를 찾습니다. 프로젝트를 다시 로드하면 NuGet 패키지 관리자를 사용하여 NuGet 패키지를 다운로드해야 하기 때문에 사용하고 있는 NuGet 패키지를 기록해 둡니다. 이 \<ItemGroup >와 함께 제거 합니다. 예를 들어 UWP 프로젝트에서 다음과 같은 NuGet 패키지를 제거해야 할 수 있습니다.
...
3. 8\.1 값을 사용 하 여 \<ApplicationTypeRevision > 요소가 포함 된 \<PropertyGroup > 요소를 찾습니다. 이 \<PropertyGroup > 요소에 대해 다음 단계를 수행 합니다.

    1. @No__t_0WindowsTargetPlatformVersion > 요소와 \<WindowsTargetPlatformMinVersion > 요소를 추가 하 고 설치한 유니버설 Windows 플랫폼 버전의 값을 제공 합니다.

    2. ApplicationTypeRevision 요소의 값을 8.1에서 10.0으로 업데이트합니다.

    3. @No__t_0MinimumVisualStudioVersion > 요소의 값을 14로 변경 합니다.

    4. @No__t_0EnableDotNetNativeCompatibleProfile > 요소를 추가 하 고 해당 값을 true로 설정 합니다.

    5. 유니버설 Windows 앱의 기본 자산 배율은 200입니다. 프로젝트에 200에서 크기가 조정 되지 않은 자산이 포함 된 경우 자산 규모의 값이 PropertyGroup > 요소를이에 \<UapDefaultAssetScale 추가 해야 합니다. [자산 및 배율](https://msdn.microsoft.com/library/jj679352.aspx)에 대해 자세히 알아보세요.

    6. Windows Phone 프로젝트의 경우 \<ApplicationType > 값을 Windows Phone에서 Windows 스토어로 변경 합니다.
...
4. @No__t_0PlatformToolset > 요소의 모든 인스턴스를 값 v140로 변경 합니다. 예를 들면,
...
5. 남아 있는 각 \<PropertyGroup > 요소에 대해 요소에 릴리스 구성이 있는 Condition 특성이 있는지 확인 합니다. 이 경우 \<UseDotNetNativeToolchain > 요소가 포함 되지 않은 경우 하나를 추가 합니다. 다음과 같이 \<UseDotNetNativeToolchain > 요소에 대 한 값을 true로 설정 합니다.
...
2. 기존 프로젝트 형식에 따라 새 스키마를 사용 하 여 \<Package > 요소를 업데이트 해야 합니다. 먼저 Windows 스토어 또는 Windows Phone 프로젝트가 있는지 여부에 따라 아래 스키마를 제거합니다.

    **Windows 스토어 프로젝트의 경우 이전:** @No__t_1Package > 요소가 다음과 같이 표시 됩니다.
    ...
    **Windows Phone 프로젝트의 경우 이전:** @No__t_1Package > 요소가 다음과 같이 표시 됩니다.
    ...
    **유니버설 Windows 플랫폼의 새:** @No__t_1Package > 요소에 아래 스키마를 추가 합니다. 방금 제거한 스키마에 대한 요소에서 연결된 네임스페이스 식별자 접두사를 모두 제거합니다. IgnorableNamespaces 속성을 uap mp로 업데이트합니다. 새 \<Package > 요소가 다음과 같이 표시 됩니다.
...
3. @No__t_1Package > 요소에 \<Dependencies > 자식 요소를 추가 합니다. 그런 다음 Name, MinVersion 및 MaxVersionTested 특성을 사용 하 여이 \<Dependencies > 요소에 \<TargetDeviceFamily > 자식 요소를 추가 합니다. Name 특성에 Windows.Universal 값을 지정합니다. MinVersion 및 MaxVersionTested에 설치한 유니버설 Windows 플랫폼 버전 값을 지정합니다. 이 요소는 다음과 유사하게 표시됩니다.
...
4. **Windows 스토어만 해당:** @No__t_2Package > 요소에 \<mp:P honeIdentity > 자식 요소를 추가 해야 합니다. PhoneProductId 특성 및 PhonePublisherId 특성을 추가합니다. @No__t_0Identity > 요소의 Name 특성과 동일한 값을 갖도록 PhoneProductId을 설정 합니다. PhonePublishedId 값을 00000000-0000-0000-0000-000000000000으로 설정합니다. 다음과 같습니다.
...
5. @No__t_0Prerequisites > 요소를 찾고이 요소와이 요소에 포함 된 모든 자식 요소를 삭제 합니다.

6. **Uap** 네임 스페이스를 다음 \<Resource > Elements: Scale, DXFeatureLevel에 추가 합니다. 예를 들면,
...
7. **Uap** 네임 스페이스를 문서 라이브러리, PicturesLibrary, VideosLibrary, MusicLibrary, Enterpriseauthentication, sharedUserCertificates, removableStorage, 약속 및 연락처 \<Capability > 요소에 추가 합니다. 예를 들면,
...
8. @No__t_1VisualElements > 요소 및 해당 자식 요소에 **uap** 네임 스페이스를 추가 합니다. 예를 들면,
...
9. @No__t_1ApplicationContentUriRules >와 모든 자식 요소에 **uap** 네임 스페이스를 추가 합니다. 예를 들면,
...
10. **Uap** 네임 스페이스를 다음 \<Extension > 요소와 모든 자식 요소에 추가 합니다. windows. p a p, appointmentsProvider windows. a p p, windows. cameraSettings, fileSavePicke, lockScreenCall, printTaskSettings, Windows.sharetarget, windows., windows., windows., windows., windows .입니다. 예를 들면,
...
11. chatMessageNotification 형식의 백그라운드 작업에 **uap** 네임스페이스를 추가합니다. 예를 들면,
...
12. 프레임워크 종속성을 변경합니다. 모든 \<PackageDependency > 요소에 게시자 이름을 추가 하 고 이미 지정 되지 않은 경우 MinVersion를 지정 합니다.
...
13. gattCharacteristicNotification 및 rfcommConnection 백그라운드 형식 작업을 Bluetooth 형식 작업으로 바꿉니다. 예를 들면,
...
14. Bluetooth 디바이스 기능 bluetooth.rfcomm 및 bluetooth.genericAttributeProfile을 제네릭 Bluetooth 기능으로 바꿉니다. 예를 들면,
...
15. 사용되지 않는 요소를 모두 제거합니다.
    ...
    1. @No__t_0VisualElements >의 이러한 특성은 더 이상 사용 되지 않으며 제거 해야 합니다.

       - @No__t_0VisualElements > 특성: Foregroundtext 특성이 있는, To 지원

       - @No__t_0DefaultTile > 특성 DefaultSize

       - @No__t_0ApplicationView > 요소
...
17. 솔루션을 다시 열려면 먼저 일부 숨겨진 파일을 제거해야 합니다.
    ...
    1. 파일 탐색기를 열고 도구 모음에서 **보기** 를 클릭한 다음 **숨겨진 항목** 및 **파일 이름 확장명**을 선택합니다. 컴퓨터에서이 폴더를 엽니다. > \\ 솔루션의 위치를 \<path 합니다. vs \\ {Project Name} \v14. 파일 확장명이 .suo인 파일이 있으면 삭제합니다.
...
4. @No__t_1TargetPlatformVersion > 및 \<TargetPlatformMinVersion > 요소를 포함 하는 \<PropertyGroup > 요소를 찾습니다. @No__t_0TargetPlatformVersion >의 기존 값을 변경 하 고 > 요소가 설치 된 유니버설 Windows 플랫폼의 동일한 버전으로 \<TargetPlatformMinVersion 합니다.

    유니버설 Windows 앱의 기본 자산 배율은 200입니다. Visual Studio 2015 RC를 사용 하 여 만든 프로젝트는 100으로 확장 된 자산을 포함 하 고 있습니다 .이 PropertyGroup에 100 값을 사용 하 여 \<UapDefaultAssetScale > 요소를 추가 해야 합니다. [자산 및 배율](https://msdn.microsoft.com/library/jj679352.aspx)에 대해 자세히 알아보세요.

5. UWP 확장 SDK(예: Windows Mobile SDK)에 참조를 추가한 경우 SDK 버전을 업데이트해야 합니다. 예를 들어 다음과 같은 \<SDKReference > 요소가 있습니다.
...
7. 다음과 같이 Microsoft \<Import를 참조 하는 프로젝트 및 조건 특성을 사용 하 여 > 요소를 찾아서 삭제 합니다.
...
8. > 자식 요소가 NuGet 패키지에 \<Reference 있는 \<ItemGroup >를 찾습니다. 이 정보는 향후 단계에 필요하므로 참조되는 NuGet 패키지를 기록해 둡니다. Visual Studio 2015 RC와 Visual Studio 2015 RTM에서 Windows 10 프로젝트 형식의 중요한 한 가지 차이점은 RTM 형식이 [NuGet](http://docs.nuget.org/) 버전 3을 사용한다는 것입니다.

    @No__t_0ItemGroup > 및 모든 자식을 제거 합니다. 예를 들어 Visual Studio RC로 만든 UWP 프로젝트에서 다음과 같은 NuGet 패키지를 제거해야 합니다.
...
9. @No__t_1AppxManifest > 요소를 포함 하는 \<ItemGroup > 요소를 찾습니다. Include 특성이로 설정 된 \<None > 요소가 있으면이를 삭제 합니다. 또한 Include 특성을 사용 하 여 \<None > 요소를 추가 하 고 해당 값을 project. json으로 설정 합니다.
...
13. 프로젝트에서 Package.appxmanifest 파일을 엽니다.
    ...
    1. @No__t_0TargetDeviceFamily > 요소를 찾습니다. MinVersion 및 MaxVersionTested 특성을 설치한 유니버설 Windows 플랫폼 버전에 맞도록 변경합니다. 다음과 같습니다.
...
3. Condition 특성을 포함 하지 않지만 \<ApplicationTypeRevision > 요소를 포함 하는 \<PropertyGroup > 요소를 찾습니다. ApplicationTypeRevision 값을 8.2에서 10.0으로 업데이트합니다. @No__t_0WindowsTargetPlatformVersion > 및 \<WindowsTargetPlatformMinVersion > 요소를 추가 하 고 사용자가 설치한 유니버설 Windows 플랫폼 버전의 값으로 해당 값을 설정 합니다.

     요소가 아직 없는 경우 \<EnableDotNetNativeCompatibleProfile > 요소를 추가 하 고 해당 값을 true로 설정 합니다.
...
4. 남아 있는 각 \<PropertyGroup > 요소에 대해 요소에 릴리스 구성이 있는 Condition 특성이 있는지 확인 합니다. 이 경우 \<UseDotNetNativeToolchain > 요소가 포함 되지 않은 경우 하나를 추가 합니다. 다음과 같이 \<UseDotNetNativeToolchain > 요소에 대 한 값을 true로 설정 합니다.
...
5. > 요소 \<EnableDotNetNativeCompatibleProfile를 업데이트 하 고 \<UseDotNetNativeToolchain > 요소를 업데이트 하 여 .NET 네이티브를 사용 하도록 설정 해야 하지만 C++ 템플릿에서는 .NET 네이티브 사용 하도록 설정 되어 있지 않습니다.
...
1. Visual Studio에서 C#/VB 단위 테스트 프로젝트를 포함하는 솔루션을 엽니다. @No__t_0OuttputType > 요소의 값을: AppContainerExe로 변경 합니다.
...
2. 이 fille에 아직 없는 경우 \<ItemGroup Label = "ProjectConfigurations" >이 요소 아래에 다음 \<ProjectConfiguration > 요소를 추가 합니다.
