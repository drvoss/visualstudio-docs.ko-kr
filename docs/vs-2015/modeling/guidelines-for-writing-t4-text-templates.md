...
이러한 일반적인 지침은 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]에서 프로그램 코드나 기타 응용 프로그램 리소스를 생성 하는 경우에 유용할 수 있습니다. 고정 규칙은 아닙니다.

## <a name="guidelines-for-design-time-t4-templates"></a>디자인 타임 T4 템플릿에 대 한 지침
 디자인 타임 T4 템플릿은 디자인 타임에 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 프로젝트에서 코드를 생성 하는 템플릿입니다. 자세한 내용은 [T4 텍스트 템플릿을 사용 하 여 디자인 타임 코드 생성](../modeling/design-time-code-generation-by-using-t4-text-templates.md)을 참조 하세요.
...
모델은 생성 되는 코드의 변수 부분에 대 한 특정 값을 가져오기 위해 각 템플릿에서 읽는 파일 또는 데이터베이스입니다. 모델은 데이터베이스, 사용자 고유 디자인, 다이어그램 또는 도메인별 언어의 XML 파일 일 수 있습니다. 일반적으로 한 모델은 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 프로젝트에서 많은 파일을 생성 하는 데 사용 됩니다. 각 파일은 별도의 템플릿에서 생성 됩니다.
...
생성 된 코드와 함께 직접 작성 하는 코드를 허용 합니다. 일반적으로 코드 생성 체계에서 발생할 수 있는 모든 변형에 대해 처리할 수 있는 것은 아닙니다. 따라서 생성 된 코드의 일부를 추가 하거나 재정의 해야 합니다. 생성 된 자료가 [!INCLUDE[csprcs](../includes/csprcs-md.md)] 또는 [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] 같은 .NET 언어의 경우 두 가지 전략이 특히 유용 합니다.
...
- 클래스는 다른 클래스에서 상속 하는 쌍으로 생성 되어야 합니다. 기본 클래스는 생성 된 모든 메서드와 속성을 포함 해야 하며, 파생 클래스는 생성자만 포함 해야 합니다. 이를 통해 직접 작성 된 코드는 생성 된 메서드를 재정의할 수 있습니다.
  ...
  공통 자료를 포함 파일 또는 런타임 템플릿으로 이동 하 여 여러 템플릿에서 유사한 텍스트와 코드 블록을 반복 하지 않도록 `<#@ include #>` 지시문을 사용 합니다. 자세한 내용은 [T4 Include 지시문](../modeling/t4-include-directive.md)을 참조 하세요.
 ...
 **MyReportText-Methods.cs**:
 ...
 사용자 지정 코드 허용: 확장 점을 제공 \< # + 클래스 기능 블록 # >에서 가상 메서드 생성을 고려 합니다. 이렇게 하면 단일 템플릿을 수정 하지 않고 많은 컨텍스트에서 사용할 수 있습니다. 템플릿을 수정 하는 대신 최소 추가 논리를 제공 하는 파생 클래스를 생성할 수 있습니다. 파생 클래스는 일반 코드 이거나 런타임 템플릿이 될 수 있습니다.
...
## <a name="guidelines-for-all-t4-templates"></a>모든 T4 템플릿에 대 한 지침
 텍스트를 분리 하 여 데이터를 수집 합니다. 계산 및 텍스트 블록을 혼합 해 서 사용 하지 마십시오. 각 텍스트 템플릿에서 첫 번째 \< # 코드 블록 # >를 사용 하 여 변수를 설정 하 고 복잡 한 계산을 수행 합니다. 첫 번째 텍스트 블록에서 템플릿 끝까지 또는 첫 번째 \< # + 클래스 기능 블록 # >에서 긴 식을 사용 하지 않고 텍스트 블록을 포함 하지 않는 한 루프 및 조건을 방지 합니다. 이 방법을 사용 하면 템플릿을 더 쉽게 읽고 유지 관리할 수 있습니다.

 포함 파일에 `.tt`를 사용 하지 마세요. 포함 파일에 대해 `.ttinclude` 같은 다른 파일 이름 확장명을 사용 합니다. 런타임 또는 디자인 타임 텍스트 템플릿으로 처리 하려는 파일에 대해서만 `.tt`을 사용 합니다. 경우에 따라 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]는 `.tt` 파일을 인식 하 고 처리를 위해 해당 속성을 자동으로 설정 합니다.
