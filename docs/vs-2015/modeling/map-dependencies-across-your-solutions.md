...
3. On the **Solution Explorer** toolbar, choose **Show on Code Map**![Create New Graph From Selected Nodes Button](../modeling/media/createnewgraphfromselectedbutton.gif "CreateNewGraphFromSelectedButton"). 또는 바로 가기 메뉴를 열고 **코드 맵에 표시**를 선택합니다. 클래스 뷰 또는 개체 브라우저에서 신규 또는 기존 코드 맵으로 항목을 끌 수도 있습니다.
...
2. 항목 및 해당 멤버를 매핑합니다. On the **Solution Explorer** toolbar click **Show on Code Map**![Create New Graph From Selected Nodes Button](../modeling/media/createnewgraphfromselectedbutton.gif "CreateNewGraphFromSelectedButton").
...
6. 맵의 항목과 관련된 추가 항목을 확인하려면 항목을 선택하고, 코드 맵 도구 모음에서 **관련 항목 표시** 를 선택하고, 맵에 추가할 관련 항목 형식을 선택합니다. 또는 항목을 하나 이상 선택하고, 바로 가기 메뉴를 열고, 맵에 추가할 관련 항목 형식에 대한 **표시...** 옵션을 선택합니다. 예를 들어 다음과 같은 가치를 제공해야 합니다.
...
|IntelliSense 검색 데이터베이스에 대한 특정 설정을 사용할 수 없습니다.|특정 IntelliSense 설정을 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]**옵션** 대화 상자에서 사용하지 못할 수 있습니다.|설정을 사용할 수 있도록 설정합니다.<br /><br /> See [Options, Text Editor, C/C++, Advanced](../ide/reference/options-text-editor-c-cpp-advanced.md).|
...
3. 코드 맵 이름을 지정합니다. Make sure that the **Save as type** box is set to **XPS files (\*.xps)** . **저장**을 선택합니다.
