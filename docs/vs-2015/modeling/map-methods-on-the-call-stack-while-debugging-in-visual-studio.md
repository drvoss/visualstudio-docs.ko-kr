...
- Visual C# .NET, Visual Basic .NET, C++, JavaScript, X++ 등 디버그할 수 있는 코드
  ...
  참조: [비디오: 코드 맵 디버거 통합으로 시각적으로 디버그 (Channel 9)](http://go.microsoft.com/fwlink/?LinkId=293418) • [호출 스택 매핑](#MapStack) • [코드에 대해 메모 하기](#MakeNotes) • [다음 호출 스택으로 맵 업데이트](#UpdateMap) • [지도에 관련 코드 추가](#AddRelatedCode) • [를 사용 하 여 버그 찾기 map](#FindBugs) • [Q & A](#QA)
