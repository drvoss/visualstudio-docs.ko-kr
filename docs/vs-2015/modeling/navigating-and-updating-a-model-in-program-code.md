...
## <a name="in-this-topic"></a>이 항목의 내용
 ...
 도형, 연결선 및 다이어그램과 모델 요소에 대 한 관계는 별도의 항목에서 설명 합니다. 자세한 내용은 [방법: @No__t_0 다이어그램을 탐색 하 고 업데이트 합니다.
...
- @No__t_0 플래그가 true 인 역할의 요소입니다. 예를 들어 요소를 표시 하는 셰이프는 삭제 됩니다.
  ...
  기본적으로 모든 포함 관계는 대상 역할에서 true를 `PropagatesDelete` 합니다. @No__t_0 삭제 해도 `familyTree` 삭제 되지 않지만 `familyTree.Delete()` 모든 `Persons`를 삭제 합니다. 자세한 내용은 [삭제 동작 사용자 지정](../modeling/customizing-deletion-behavior.md)을 참조 하세요.
...
## <a name="see-also"></a>관련 항목:
 도메인별 [언어에서 코드를 생성](../modeling/generating-code-from-a-domain-specific-language.md) 하 [는 도메인 특정 언어의 유효성 검사](../modeling/validation-in-a-domain-specific-language.md) 를 <xref:Microsoft.VisualStudio.Modeling.ModelElement> [How: 트랜잭션을 사용 하 여 [Visual Studio Modelbus](../modeling/integrating-models-by-using-visual-studio-modelbus.md) [변경 내용에 대 한 응답 및 전파](../modeling/responding-to-and-propagating-changes.md) 를 사용 하 여 모델 통합 ](../modeling/how-to-use-transactions-to-update-the-model.md) 모델을 업데이트 합니다.
