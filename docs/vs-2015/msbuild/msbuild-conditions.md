...
|조건|설명|  
|---------------|-----------------|  
|'`stringA`' == '`stringB`'|`true`가 `stringA`와 같으면 `stringB`로 평가됩니다.<br /><br /> 예를 들면 다음과 같습니다.<br /><br /> `Condition="'$(CONFIG)'=='DEBUG'"`<br /><br /> 간단한 영숫자 문자열 또는 부울 값에는 작은따옴표가 필요하지 않습니다. 그러나 빈 값에는 작은따옴표가 필요합니다.|  
|'`stringA`' != '`stringB`'|`true`가 `stringA`와 같지 않으면 `stringB`로 평가됩니다.<br /><br /> 예를 들면 다음과 같습니다.<br /><br /> `Condition="'$(CONFIG)'!='DEBUG'"`<br /><br /> 간단한 영숫자 문자열 또는 부울 값에는 작은따옴표가 필요하지 않습니다. 그러나 빈 값에는 작은따옴표가 필요합니다.|  
...
|Exists('`stringA`')|이름이 `true`인 파일이나 폴더가 있으면 `stringA`로 평가됩니다.<br /><br /> 예를 들면 다음과 같습니다.<br /><br /> `Condition="!Exists('$(builtdir)')"`<br /><br /> 간단한 영숫자 문자열 또는 부울 값에는 작은따옴표가 필요하지 않습니다. 그러나 빈 값에는 작은따옴표가 필요합니다.|  
|HasTrailingSlash('`stringA`')|지정한 문자열에 후행 백슬래시(`true`) 또는 슬래시(/) 문자가 있는 경우 \\로 평가됩니다.<br /><br /> 예를 들면 다음과 같습니다.<br /><br /> `Condition="!HasTrailingSlash('$(OutputPath)')"`<br /><br /> 간단한 영숫자 문자열 또는 부울 값에는 작은따옴표가 필요하지 않습니다. 그러나 빈 값에는 작은따옴표가 필요합니다.|  
|!|피연산자가 `true`로 평가되면 `false`로 평가됩니다.|  
|및|피연산자가 `true`로 평가되면 `true`로 평가됩니다.|  
