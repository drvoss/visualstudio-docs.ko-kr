...
- UI 맵(UIMap.uitest)에 `AssertForAddTwoNumbers` 메서드를 추가합니다. 이 파일은 어설션을 편집할 수 있는 코딩된 UI 테스트 편집기에서 열립니다.
  ...
  *코딩 된 UI 테스트 빌더에서 어설션 추가 도구를 선택 하려고 하면 선택 하려는 컨트롤이 포커스를 잃고 사라집니다. 컨트롤을 선택 어떻게 할까요??* **키보드를 사용 하 여 숨겨진 컨트롤을 선택**
...
- **코드 편집기:**
  ...
  - 코딩된 UI 테스트에는 Internet Explorer 9 및 Internet Explorer 10에 포함된 일부 HTML5 컨트롤에 대한 지원이 포함됩니다. 자세한 내용은 [Using HTML5 Controls in Coded UI Tests](../test/using-html5-controls-in-coded-ui-tests.md)을 참조하세요.
...
- [방법: 데스크톱과 상호 작용하는 테스트를 실행하기 위해 테스트 에이전트 설정](/visualstudio/test/how-to-set-up-your-test-agent-to-run-tests-that-interact-with-the-desktop?view=vs-2015)
