...
|<xref:System.Activities.Statements.State.Entry%2A>|False|이 상태가 전환될 때 발생하는 동작을 지정합니다. @No__t_0 활동이 확장 되 면 **도구 상자** 에서 활동을 끌어 상태의 **항목** 섹션에 놓으면이 값을 설정할 수 있습니다.|
|<xref:System.Activities.Statements.State.Exit%2A>|False|이 상태가 다른 상태로 전환될 때 발생하는 동작을 지정합니다. @No__t_0 활동이 확장 되 면 **도구 상자** 에서 활동을 끌어 상태의 **종료** 섹션에 놓으면이 값을 설정할 수 있습니다.|
