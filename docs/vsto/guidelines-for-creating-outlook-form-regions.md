...
## <a name="DisablingInheritance"></a>양식 영역 상속 사용 안 함
 ...
 양식 영역을 파생된 메시지 클래스와 연결하지 않으려면 양식 영역의 <xref:Microsoft.Office.Tools.Outlook.FormRegionManifest.ExactMessageClass%2A> 속성을 **true**를 참조하세요. 예를 들어 인접 양식 영역을에 `IPM.Task` 연결 하 고 <xref:Microsoft.Office.Tools.Outlook.FormRegionManifest.ExactMessageClass%2A> 속성을 **true**로 설정 하는 경우 양식 영역은 표준 작업 폼의 아래쪽에만 추가 됩니다. 양식 영역이 사용자 지정된 버전의 표준 작업 양식 하단에는 추가되지 않습니다.
