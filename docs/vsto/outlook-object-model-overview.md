...
- 메일 메시지 또는 모임 요청과 같은 단일 항목의 내용을 표시하는 창에 액세스하는 데 사용할 수 있는 <xref:Microsoft.Office.Interop.Outlook._Application.Inspectors%2A> 속성
  ...
  <xref:Microsoft.Office.Interop.Outlook.Application> 개체의 인스턴스를 가져오려면 프로젝트에서 `ThisAddIn` 클래스의 응용 프로그램 필드를 사용 합니다. 자세한 내용은 [VSTO 추가 기능 프로그래밍](../vsto/programming-vsto-add-ins.md)을 참조 하세요.

> [!NOTE]
> Outlook 개체 모델 보호에 의해 차단 되는 속성 및 메서드를 사용 하는 경우 보안 경고를 방지 하려면 `ThisAddIn` 클래스의 응용 프로그램 필드에서 Outlook 개체를 가져옵니다. 자세한 내용은 [Office 솔루션에 대 한 특정 보안 고려 사항](../vsto/specific-security-considerations-for-office-solutions.md)을 참조 하세요.
...
### <a name="folder-object"></a>Folder 개체
 ...
 기본 <xref:Microsoft.Office.Interop.Outlook.Folder>에 액세스 하 고 새 <xref:Microsoft.Office.Interop.Outlook.Folder>을 만드는 방법을 보여 주는 예제는 [방법: 프로그래밍 방식으로 사용자 지정 폴더 항목 만들기](../vsto/how-to-programmatically-create-custom-folder-items.md)를 참조 하세요.
...
### <a name="vba-object-model-reference"></a>VBA 개체 모델 참조
 ...
 VBA 개체 모델 참조의 모든 개체 및 멤버는 Outlook PIA의 형식 및 멤버에 해당합니다. 예를 들어 VBA 개체 모델 참조의 Inspector 개체는 Outlook PIA의 <xref:Microsoft.Office.Interop.Outlook.Inspector> 개체에 해당 합니다. VBA 개체 모델 참조에서는 대부분의 속성, 메서드 및 이벤트에 대한 코드 예제를 제공하지만 Visual Studio를 사용하여 만든 Outlook VSTO 추가 기능 프로젝트에서 사용하려면 이 참조의 VBA 코드를 Visual Basic 또는 Visual C#으로 변환해야 합니다.
