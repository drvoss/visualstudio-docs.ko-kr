...
@No__t_0 활동을 추가 하려면 **도구 상자** 의 **상태 시스템** 섹션에서 **StateMachine** 활동 디자이너를 끌어 워크플로 디자이너 화면에 놓습니다. 이 <xref:System.Activities.Statements.StateMachine> 작업에 자식 상태를 추가 하려면 **도구 상자** 에서 <xref:System.Activities.Statements.State> 또는 <xref:System.Activities.Core.Presentation.FinalState>를 끌어 **StateMachine**에 놓습니다.
